...
export default function page() {
  ...
  useEffect(() => {
    async function fetchTasks() {
      const { data, error } = await supabase
        .from("categories_tb")
        .select("cat_id, category_name, cat_image_url, created_at")
  ...
  const handleAddVocab = () => {
    router.push(`/addcat/${staff?.staff_id}`);
  };
                ...
                <tbody className="bg-white divide-y divide-gray-400">
                  {cats.map((cat) => (
                    ...
                    >
                      ...
                      <td className="border-b border-r border-gray-400 p-3 text-sm font-semibold text-gray-700 text-center">
                        {new Date(cat.created_at).toLocaleDateString()}
                      </td>
                      <td className="border-b border-r border-gray-400 p-3 text-sm font-semibold text-gray-700 text-center">
                        <Link
                          href={`/editcat/${cat.cat_id}?staff_id=${staff?.staff_id}`}
